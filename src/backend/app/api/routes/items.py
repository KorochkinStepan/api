...
from app.models import Item, ItemCreate, ItemOut, ItemsOut, ItemUpdate, MessageInfo
...
@router.delete("/{id}")
def delete_item(session: SessionDep, current_user: CurrentUser, id: int) -> MessageInfo:
    """
    Delete an item.
    """
    item = session.get(Item, id)
    ...
    session.delete(item)
    session.commit()
    return MessageInfo(message="Item deleted successfully")
